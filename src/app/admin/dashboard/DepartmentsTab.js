...
export default function DepartmentsTab() {
  ...
  useEffect(() => {
    fetch("https://167.86.74.16:5000/departments")
      .then((res) => res.json())
  ...
  const handleSaveEdit = (index) => {
    if (editedName.trim() !== "") {
      fetch(`https://167.86.74.16:5000/departments/${departments[index].name}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDelete = (name) => {
    fetch(`https://167.86.74.16:5000/departments/${name}`, { method: "DELETE" })
